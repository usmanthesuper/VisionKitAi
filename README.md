...
<p align="center">
    ...
    <img width="100" src="https://github.com/roboflow-ai/notebooks/raw/main/assets/roboflow_logomark_color.svg" alt="visionkit logo">
    </br>
</p>
...
```
vk.load(model: model, modelVersion: modelVersion) { [self] model, error, modelName, modelType in
    mlModel = model
    if error != nil {
...
```
vk.uploadImage(image: image, project: project) { result in
...
An example app can be found [here](https://github.com/yourusername/visionkit-swift-examples) that illusrates how to use the Vision Kit SDK on an iOS app. The app uses apre-trained model hosted on Roboflow Universe for detecting the actions in a round of rock-paper-scissors. You'll have to provide your own API key. 
...
To install the Vision Kit Swift SDK package into your packages, add a reference to the Vision Kit Swift SDK and a targeting release version in the dependencies section in `Package.swift` file:
...
let package = Package(
    ...
    products: [],
    dependencies: [
        .package(url: "https://github.com/yourusername/visionkit-swift", from: "1.0.0")
    ]
)
```
...
- Then add https://github.com/yourusername/visionkit-swift



#### Cocoapods ####

To install with Cocoapods, make sure you have Cocoapods already installed and added to your project, and then run `pod VisionKit` to your podfile: 

Then, run `pod install` in the root directory of your project. 

If you've previously installed the Vision Kit SDK via Cocoapods, you'll need to update your podfile to have an entry of `pod VisionKit`. 
...
xcodebuild test -scheme VisionKitTests -destination 'platform=macOS,variant=Mac Catalyst,arch=arm64'
```

The test suite includes:
- **Model Loading Tests**: Validates that object detection and segmentation models load correctly from the Vision Kit API
...
1. Update the version number in `VisionKit.podspec`
