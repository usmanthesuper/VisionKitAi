...
public class VKModel: NSObject {
    ...
    public func detect(pixelBuffer: CVPixelBuffer, completion: @escaping (([VKPrediction]?, Error?) -> Void)) {
        completion(nil, NSError(domain: "VKModel", code: -1, userInfo: [NSLocalizedDescriptionKey: "Error loading model"]))
    }
 
    public func detect(pixelBuffer: CVPixelBuffer) async -> ([VKPrediction]?, Error?) {
