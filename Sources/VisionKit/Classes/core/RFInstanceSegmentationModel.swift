//
//  VKInstanceSegmentationModel.swift
//  VisionKit
//
...
public class VKInstanceSegmentationModel: VKObjectDetectionModel {
    
    
    //Load the retrieved CoreML model into an already created VKInstanceSegmentationModel instance
    ...
    public override func detect(pixelBuffer:CVPixelBuffer, completion: @escaping (([VKPrediction]?, Error?) -> Void)) {
        ...
        do {
            ...
            var final: [VKObjectDetectionPrediction] = []
            ...
            if #available(iOS 18.0, macOS 15.0, *) {
                ...
                for (i, maskBin) in maskBins.enumerated() {
                    ...
                    let detection = VKInstanceSegmentationPrediction(x: Float(box.midX) * scaleX, y: Float(box.midY) * scaleY, width: Float(box.width) * scaleX, height: Float(box.height) * scaleY, className: classname, confidence: keep[4], color: hexStringToCGColor(hex: colors[classname] ?? "#ff0000"), box: box, points:polygon)
