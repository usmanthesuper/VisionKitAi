...
public class VKClassificationModel: VKModel {
    ...
    public override func detect(pixelBuffer buffer: CVPixelBuffer, completion: @escaping (([VKPrediction]?, Error?) -> Void)) {
        guard let mlModel = self.mlModel else {
            completion(nil, NSError(domain: "VKClassificationModel", code: 1, userInfo: [NSLocalizedDescriptionKey: "Model initialization failed."]))
            return
        }
        ...
        do {
            ...
            guard let inputName = mlModel.modelDescription.inputDescriptionsByName.keys.first,
                ...
                completion(nil, NSError(domain: "VKClassificationModel", code: 2, userInfo: [NSLocalizedDescriptionKey: "Could not find model input name"]))
                return
            }
            ...
            guard let outputName = mlModel.modelDescription.outputDescriptionsByName.keys.first,
                ...
                completion(nil, NSError(domain: "VKClassificationModel", code: 3, userInfo: [NSLocalizedDescriptionKey: "Could not find model output"]))
                return
            }
            
            var predictions: [VKPrediction] = []
            ...
            if let multiArray = output.multiArrayValue {
                ...
                for i in 0..<multiArray.count {
                    ...
                    if confidence >= Float(threshold) {
                        let prediction = VKClassificationPrediction(
            ...
            // Sort by confidence (highest first) - cast to VKClassificationPrediction since that's what we create
            predictions.sort { 
                guard let pred1 = $0 as? VKClassificationPrediction, 
                    let pred2 = $1 as? VKClassificationPrediction else { 
                    return false 
                }
