//
//  VKSDKObjectDetectionModel.swift
//  VisionKit
//
...
import CoreML
import Vision
//Creates an instance of an ML model for object detection
public class VKSDKObjectDetectionModel: VKSDKModel {
    ...
    public override func detect(pixelBuffer buffer: CVPixelBuffer, completion: @escaping (([VKSDKPrediction]?, Error?) -> Void)) {
        ...
        do {
            ...
            for detectResult in detectResults {
                ...
                let detection = VKSDKObjectDetectionPrediction(x: Float((box.maxX+box.minX)/2.0), y: Float((box.maxY+box.minY)/2.0), width: Float((box.maxX-box.minX)), height: Float((box.maxY-box.minY)), className: label, confidence: confidence, color: hexStringToCGColor(hex: colors[label] ?? "#ff0000"), box: box)
