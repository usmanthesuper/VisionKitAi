...
public class VKObjectDetectionModel: VKModel {
    ...
    public override func detect(pixelBuffer buffer: CVPixelBuffer, completion: @escaping (([VKPrediction]?, Error?) -> Void)) {
        ...
        do {
            ...
            for detectResult in detectResults {
                ...
                let detection = VKObjectDetectionPrediction(x: Float((box.maxX+box.minX)/2.0), y: Float((box.maxY+box.minY)/2.0), width: Float((box.maxX-box.minX)), height: Float((box.maxY-box.minY)), className: label, confidence: confidence, color: hexStringToCGColor(hex: colors[label] ?? "#ff0000"), box: box)
