//
//  VKSDKDetrObjectDetectionModel.swift
//  VisionKit
//
...
/// Object detection model that uses DETR for inference
public class VKSDKDetrObjectDetectionModel: VKSDKObjectDetectionModel {

    public override init() {
        super.init()
    }
    
    /// Load the retrieved CoreML model for DETR
    ...
    public override func detect(pixelBuffer buffer: CVPixelBuffer, completion: @escaping (([VKSDKPrediction]?, Error?) -> Void)) {
    ...
    private func detectWithVNCoreML(pixelBuffer buffer: CVPixelBuffer, completion: @escaping (([VKSDKPrediction]?, Error?) -> Void)) {
        guard let coreMLRequest = self.coreMLRequest else {
            completion(nil, NSError(domain: "VKSDKDetrObjectDetectionModel", code: 1, userInfo: [NSLocalizedDescriptionKey: "VNCoreML model initialization failed."]))
            return
        }
        ...
        do {
            ...
            guard let results = coreMLRequest.results as? [VNCoreMLFeatureValueObservation] else {
                completion(nil, NSError(domain: "VKSDKDetrObjectDetectionModel", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to get RFDetr model outputs"]))
                return
            }
            ...
            guard let boxesArray = boxes,
                ...
                completion(nil, NSError(domain: "VKSDKDetrObjectDetectionModel", code: 3, userInfo: [NSLocalizedDescriptionKey: "Missing required RFDetr outputs (boxes, scores, labels)"]))
                return
            }
            
            // Process RFDetr outputs to create detection objects
            let detections = try processDETROutputs(
                boxes: boxesArray,
                scores: scoresArray,
                labels: labelsArray,
    ...
    /// Process DETR raw outputs into VKSDKObjectDetectionPrediction objects
    private func processDETROutputs(boxes: MLMultiArray, scores: MLMultiArray, labels: MLMultiArray, imageWidth: Int, imageHeight: Int) throws -> [VKSDKObjectDetectionPrediction] {
        var detections: [VKSDKObjectDetectionPrediction] = []
        ...
        for i in 0..<numDetections {
            ...
            let detection = VKSDKObjectDetectionPrediction(
                x: centerX,
                y: centerY,
                width: width,
                height: height,
