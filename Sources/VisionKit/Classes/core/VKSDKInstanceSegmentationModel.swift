//
//  VKSDKInstanceSegmentationModel.swift
//  VisionKit
//
...
//Creates an instance of an ML model for instance segmentation
public class VKSDKInstanceSegmentationModel: VKSDKObjectDetectionModel {
    
    
    //Load the retrieved CoreML model into an already created VKSDKInstanceSegmentationModel instance
    ...
    public override func detect(pixelBuffer:CVPixelBuffer, completion: @escaping (([VKSDKPrediction]?, Error?) -> Void)) {
        ...
        do {
            ...
            var final: [VKSDKObjectDetectionPrediction] = []
            ...
            if #available(iOS 18.0, macOS 15.0, *) {
                ...
                for (i, maskBin) in maskBins.enumerated() {
                    ...
                    let detection = VKSDKInstanceSegmentationPrediction(x: Float(box.midX) * scaleX, y: Float(box.midY) * scaleY, width: Float(box.width) * scaleX, height: Float(box.height) * scaleY, className: classname, confidence: keep[4], color: hexStringToCGColor(hex: colors[classname] ?? "#ff0000"), box: box, points:polygon)
