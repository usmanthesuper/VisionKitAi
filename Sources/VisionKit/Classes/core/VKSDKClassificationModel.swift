//
//  VKSDKClassificationModel.swift
//  VisionKit
//
...
public class VKSDKClassificationModel: VKSDKModel {
    ...
    //Load the retrieved CoreML model into an already created VKClassificationModel instance
    ...
    public override func detect(pixelBuffer buffer: CVPixelBuffer, completion: @escaping (([VKSDKPrediction]?, Error?) -> Void)) {
        guard let mlModel = self.mlModel else {
            completion(nil, NSError(domain: "VKSDKClassificationModel", code: 1, userInfo: [NSLocalizedDescriptionKey: "Model initialization failed."]))
            return
        }
        ...
        do {
            ...
            guard let inputName = mlModel.modelDescription.inputDescriptionsByName.keys.first,
                ...
                completion(nil, NSError(domain: "VKSDKClassificationModel", code: 2, userInfo: [NSLocalizedDescriptionKey: "Could not find model input name"]))
                return
            }
            ...
            guard let outputName = mlModel.modelDescription.outputDescriptionsByName.keys.first,
                ...
                completion(nil, NSError(domain: "VKSDKClassificationModel", code: 3, userInfo: [NSLocalizedDescriptionKey: "Could not find model output"]))
                return
            }
            
            var predictions: [VKSDKPrediction] = []
            ...
            if let multiArray = output.multiArrayValue {
                ...
                for i in 0..<multiArray.count {
                    ...
                    if confidence >= Float(threshold) {
                        let prediction = VKSDKClassificationPrediction(
            ...
            // Sort by confidence (highest first) - cast to VKSDKClassificationPrediction since that's what we create
            predictions.sort { 
                guard let pred1 = $0 as? VKSDKClassificationPrediction, 
                    let pred2 = $1 as? VKSDKClassificationPrediction else { 
                    return false 
                }
