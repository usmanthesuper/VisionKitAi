...
final class ObjectDetectionTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        vk.clearModelCache(modelName: "playing-cards-ow27d", modelVersion: 2)
        vk.clearModelCache(modelName: "hard-hat-sample-txcpu", modelVersion: 7)
    }
    ...
    func testLoadModel() async {
        let vk = VisionKitMobile(apiKey: API_KEY)
        let (model, error, _, _) = await vk.load(model: "playing-cards-ow27d", modelVersion: 2)
        self.model = model
        XCTAssertNil(error)
    ...
    func testLoadRFDetrModel() async {
        guard let model = await TestUtils.loadDETRModel() else {
            XCTFail("Failed to load RFDetr model")
            return
        }
